...
# Đồ thị với heuristic
graph = {
    'A': [['C', 33]],
    'B': [['C', 22], ['D', 18], ['E', 38], ['F', 31]],
    'C': [['A', 33], ['B', 22], ['D', 20], ['E', 28], ['F', 24]],
    'D': [['B', 18], ['C', 20], ['E', 23], ['F', 37]],
    'E': [['B', 38], ['C', 28], ['D', 23], ['F', 21]],
...
def greedy_best_first_search(graph, start):
    ...
    while len(visited) < len(graph):
        ...
        _, cost_next_node, next_node = heapq.heappop(heap)
        total_cost += cost_next_node
...
if __name__ == "__main__":
    ...
    if start not in graph:
        ...
        if path is None:
            print(">> Không thể tìm thấy đường đi để quay về điểm xuất phát.")
        else:
